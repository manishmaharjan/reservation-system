...
def test_update_user_invalid_email(client):
    ...
    headers = {"api_key": api_key}
    response = client.put(f"/api/users/{user_id}/", json=invalid_user_data, headers=headers)
...
def test_create_user_duplicate_username(client):
    ...
    response = client.post("/api/users/", json=user_data)  # Second creation with same username should fail
    assert response.status_code == 409
