...
def test_create_get_reservation(client):
    ...
    response = client.get(f"/api/users/{user_id}/reservations/{reservation_id}/", headers = headers)
...
def test_invalid_userid(client):
    api_key, user_id = create_user(client)
    headers = {"Api_key": api_key}

    response = client.get(f"/api/users/-4/reservations/1/", headers = headers)
    assert response.status_code == 400
    assert response.text == "Invalid userId parameter"
    ...
    response = client.put(f"/api/users/invalid/reservations/1/", headers = headers, json = {})
    assert response.status_code == 400
    assert response.text == "Invalid userId parameter"
...
def test_invalid_userid_collection(client):
    api_key, user_id = create_user(client)
    headers = {"Api_key": api_key}
...
def test_api_key_id_not_matching_normal(client):
    ...
    response = client.put(f"/api/users/{user_id}/reservations/1/", headers=headers, json = {})
    assert response.status_code == 401
    assert response.text == "The provided Api-key does not correspond to the userId provided."
...
def test_api_key_id_not_matching_collection(client):
    ...
    response = client.post(f"/api/users/{user_id}/reservations/", headers=headers, json = {})
    assert response.status_code == 401
    assert response.text == "The provided Api-key does not correspond to the userId provided."
...
def test_update_reservation_success(client):
    ...
    response = client.put(f"/api/users/{user_id}/reservations/{reservation_id}/", json=updated_reservation_data, headers=headers)
    assert response.status_code == 200
...
def test_update_reservation_invalid_user_id(client):
    ...
    response = client.put(f"/api/users/-4/reservations/{reservation_id}/", json={}, headers=headers)
    assert response.status_code == 400
    assert response.text == "Invalid userId parameter"
...
def test_update_reservation_invalid_room_id(client):
    ...
    invalid_room_data = {
        "roomId": 999  # Non-existent room ID
    }
    response = client.put(f"/api/users/{user_id}/reservations/{reservation_id}/", json=invalid_room_data, headers=headers)
...
def test_get_update_delete_unbelonging_reservation(client):
    ...
    reservation_id = create_reservation(client, api_key, user_id)
    response = client.get(f"/api/users/{user_id1}/reservations/{reservation_id}/", headers = headers)
    assert response.status_code == 403
    assert response.text == "Reservation does not belong to the provided userId."

    response = client.put(f"/api/users/{user_id1}/reservations/{reservation_id}/", headers = headers, json = {})
    assert response.status_code == 403
    assert response.text == "Reservation does not belong to the provided userId."

    response = client.delete(f"/api/users/{user_id1}/reservations/{reservation_id}/", headers = headers, json = {})
    assert response.status_code == 403
    assert response.text == "Reservation does not belong to the provided userId."
...
def test_get_update_delete_no_existing_reservation(client):
    ...
    assert response.status_code == 404
    assert response.text == "No reservation found with the provided reservationId."

    response = client.put(f"/api/users/{user_id}/reservations/9999/", headers = headers, json = {})
    assert response.status_code == 404
    assert response.text == "No reservation found with the provided reservationId."

    response = client.delete(f"/api/users/{user_id}/reservations/9999/", headers = headers)
    assert response.status_code == 404
    assert response.text == "No reservation found with the provided reservationId."
...
def test_delete_reservation(client):
    ...
    reservation_id = create_reservation(client, api_key, user_id)
    response = client.delete(f"/api/users/{user_id}/reservations/{reservation_id}/", headers = headers)
    assert response.status_code == 200

def test_invalid_reservationid(client):
    api_key, user_id = create_user(client)
    headers = {"Api_key": api_key}

    response = client.get(f"/api/users/{user_id}/reservations/-1/", headers = headers)
    assert response.status_code == 400
    assert response.text == "Invalid reservationId parameter"

    response = client.get(f"/api/users/{user_id}/reservations/invalid/", headers = headers)
    assert response.status_code == 400
    assert response.text == "Invalid reservationId parameter"
    ...
    response = client.put(f"/api/users/{user_id}/reservations/invalid/", headers = headers)
    assert response.status_code == 400
    assert response.text == "Invalid reservationId parameter"
    ...
    response = client.delete(f"/api/users/{user_id}/reservations/invalid/", headers = headers)
    assert response.status_code == 400
    assert response.text == "Invalid reservationId parameter"

def test_invalid_reservationid_collection(client):
    api_key, user_id = create_user(client)
    headers = {"Api_key": api_key}
...
def test_put_not_json_format(client):
    ...
    response = client.put(f"/api/users/{user_id}/reservations/{reservation_id}/", headers = headers, data = "torve")
...
def test_put_error_parsing_data(client):
    ...
    response = client.put(f"/api/users/{user_id}/reservations/{reservation_id}/", headers = headers, json = "torve")
    assert response.status_code == 400 
...
def test_post_error_parsing_data(client):
    ...
    response = client.post(f"/api/users/{user_id}/reservations/", headers = headers, json = "torve")
    assert response.status_code == 400 
    assert response.text == "Error parsing JSON data"
...
def test_put_at_least_one_field(client):
    ...
    reservation_id = create_reservation(client, api_key, user_id)
    data = {}
    response = client.put(f"/api/users/{user_id}/reservations/{reservation_id}/", headers = headers, json = data)

    assert response.status_code == 400 
    assert response.text == "At least one of date, start-time, end-time, or roomId is required."
...
def test_put_invalid_time_or_date(client):
    ...
    assert response.status_code == 400 
    assert response.text == "Invalid date or time format. Date format: YYYY-MM-DD. Time format: HH:MM"
...
def test_put_time_slot_taken(client):
    api_key, user_id = create_user(client)
    headers = {"Api-key": api_key}
    reservation_id = create_reservation(client, api_key, user_id, start_time="11:00", end_time="12:00")
    reservation_id1 = create_reservation(client, api_key, user_id, start_time="13:00", end_time="14:00")
    data = {"start-time": "10:00", "end-time": "11:01"}
    response = client.put(f"/api/users/{user_id}/reservations/{reservation_id1}/", headers = headers, json = data)
...
def test_get_reservations(client):
    ...
    # Test invalid userId scenario
    response = client.get("/api/users/-4/reservations/", headers=headers)
    assert response.status_code == 400
    assert response.text == "Invalid userId parameter"

    # Test API key does not correspond to userId scenario
    headers = {"Api-key": "invalidApikey"}
    response = client.get(f"/api/users/{user_id}/reservations/", headers=headers)
    assert response.status_code == 401
    assert response.text == "Incorrect api key."

def test_post_invalid_userId(client):
    api_key, user_id = create_user(client)
    headers = {"Api-key": api_key}
...
def test_create_reservation(client):
    ...
    response = client.post(f"/api/users/{user_id}/reservations/", json=invalid_reservation_data, headers=headers)
    assert response.status_code == 400
    ...
    response = client.post(f"/api/users/{user_id}/reservations/", json=invalid_room_reservation_data, headers=headers)
    assert response.status_code == 404
    assert response.text == "No room found with the provided room id."
    ...
    response = client.post(f"/api/users/{user_id}/reservations/", json=past_time_reservation_data, headers=headers)
    assert response.status_code == 409
    ...
    response = client.post(f"/api/users/{user_id}/reservations/", json=conflicting_reservation_data, headers=headers)
    assert response.status_code == 409
    assert response.text == "Time slot already taken"
...
def test_post_invalid_time_or_date(client):
    ...
    response = client.post(f"/api/users/{user_id}/reservations/", headers = headers, json = data)
